...
const HIRE_COLORS = {
  'EMP-001': '#378ADD', 'EMP-002': '#7077A1', 'EMP-003': '#F87171',
  'EMP-004': '#1D9E75', 'EMP-005': '#F6B17A', 'EMP-006': '#6C5CE7',
  'EMP-007': '#2D3250', 'EMP-008': '#A8D5BA',
};
...
const CUSTOM_DOT = (props) => {
  ...
  return (
    <g>
      <circle cx={cx} cy={cy} r={payload.z / 4 + 6} fill={HIRE_COLORS[payload.id] || '#7077A1'} fillOpacity={0.7} stroke={isAtRisk ? '#EF4444' : 'white'} strokeWidth={isAtRisk ? 2 : 1} />
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
        ...
        <ResponsiveContainer width="100%" height={240}>
          <BarChart data={velocityData} margin={{ left: 0, right: 16, top: 4, bottom: 4 }}>
            ...
            <Tooltip formatter={(v, n) => [`${v}%`, n === 'actual' ? 'Actual' : 'Expected']} contentStyle={{ borderRadius: 12, border: '1px solid #E5E7EB', fontSize: 12 }} />
            <Legend formatter={v => v === 'actual' ? 'Actual %' : 'Expected %'} />
            <Bar dataKey="expected" fill="#E5E7EB" radius={[4, 4, 0, 0]} maxBarSize={28} name="expected" />
            <Bar dataKey="actual" radius={[4, 4, 0, 0]} maxBarSize={28} name="actual">
              {velocityData.map((entry, i) => (
                <Cell key={i} fill={entry.diff >= 0 ? '#1D9E75' : '#F87171'} />
              ))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          ...
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={productivityData} layout="vertical" margin={{ left: 8, right: 24, top: 0, bottom: 0 }}>
              ...
              <Tooltip formatter={(v) => [`${v} days`, 'Avg Duration']} contentStyle={{ borderRadius: 12, border: '1px solid #E5E7EB', fontSize: 12 }} />
              <Bar dataKey="days" radius={[0, 6, 6, 0]} maxBarSize={18}>
                {productivityData.map((_, i) => (
                  <Cell key={i} fill={i < 3 ? '#1D9E75' : i > 5 ? '#F87171' : '#378ADD'} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
              ...
              <ScatterChart margin={{ left: 8, right: 24, top: 24, bottom: 24 }}>
                ...
                <Tooltip
                  ...
                  content={({ active, payload }) => {
                    ...
                    return (
                      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-3 text-xs">
                ...
                <ReferenceLine y={6} stroke="#E5E7EB" strokeDasharray="4 4" />
                <Scatter data={scatterData} shape={CUSTOM_DOT} />
              </ScatterChart>
            </ResponsiveContainer>
            {/* Quadrant labels */}
            <div className="absolute top-6 left-12 text-[9px] font-medium text-gray-300 bg-green-50 rounded px-1.5 py-0.5">Engaged but stuck</div>
            <div className="absolute top-6 right-8 text-[9px] font-medium text-green-600 bg-green-50 rounded px-1.5 py-0.5">✦ Thriving</div>
